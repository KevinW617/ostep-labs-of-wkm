...
int launch_process(char **tokens)
{
    ...
    //     if(tokens[0][0]=='/')
    //     {
    //         strcpy(exe_path,tokens[0]);//输入为绝对路径时
    //     }
    //     else
    //     {//相对路径
    ...
    //         while(dir!=NULL)
    //         {
    //             sprintf(exe_path,"%s/%s",dir,tokens[0]);
...
//             // 只有child process才执行，父进程跳过
//             if (execvp(exe, cmds[i]) == -1)
//             {
//                 //printError();
//                 perror("execvp");
//                 exit(1);
...
int action(char **tokens, int i)
{
    ...
    //     else
    //     {
    //         printf("Error: Missing directory argument for cd.\n");
    //     }
    // }
    // else
    // {
...
char **tokenize_input(char *input,int *num_tokens)
{
    ...
    for(int j = 0;input[j]!='\0';j++ )
    {
        if(input[j]==' ')
        {
            if(token_end>=token_start && input[token_start]!=' ')//避免连续空格
    ...
    *num_tokens = i;
    ...
    //         {
    //             tokens[i] = malloc((j-token_start + 1)*sizeof(char));
    //             strncpy(tokens[i],input + token_start,j-token_start);
    //             tokens[i][j-token_start]='\0';
    //             i++;
    //             token_start = -1;
    //         }
    ...
    //     }
    // }
    // if(token_start !=-1 && in_token)
    // {
    //     tokens[i] = malloc((strlen(input + token_start)+1)*sizeof(char));
    //     strcpy(tokens[i],input + token_start);
    //     i++;
    // }
...
int main(int argc, char **argv)
{
    ...
    if (is_batch_mode)
    {
        ...
        do
        {
            ...
            // while ((token = strsep(&line_without_newline, " ")) != NULL) // strsep函数会返回指向每个子字符串的指针，而不是复制每个子字符串的内容。这意味着 token 指针仅仅是指向原始输入字符串中的一部分，而不是新分配的内存
            // {                                                            // 使用复制的行来分割标记
            //     if (strlen(token) > 0)
            //     {
